    ...
    async function getAdapter(appOptions: { session?: any; baseUrl?: any }) {
        ...
        async function getSession(sessionToken: ExprArg) {
            ...
            try {
                ...
                const session = await faunaClient.query({
                    ...
                    expires: q.ToMillis(
                        q.Select(['data', 'expires'], sessionFQL)
                    ),
                    sessionToken: q.Select(
                        ['data', 'sessionToken'],
                        sessionFQL
                    ),
                    accessToken: q.Select(['data', 'accessToken'], sessionFQL),
                    createdAt: q.ToMillis(
                        q.Select(['data', 'createdAt'], sessionFQL)
                    ),
                    updatedAt: q.ToMillis(
                        q.Select(['data', 'updatedAt'], sessionFQL)
                    ),
                });
        ...
        async function createVerificationRequest(
            ...
            try {
                ...
                return Promise.reject(
                    new Error('CREATE_VERIFICATION_REQUEST_ERROR')
                );
            }
        }
        ...
        async function getVerificationRequest(
            ...
            const FQL = q.Let(
                {
                    ref: q.Match(
                        q.Index(indexes.VerificationRequest),
                        hashedToken
                    ),
                },
                q.If(
            ...
            try {
                ...
                return Promise.reject(
                    new Error('GET_VERIFICATION_REQUEST_ERROR')
                );
            }
        }
